...
// Import images
import image1 from '../image/music1.webp';
import image2 from '../image/music2.webp';
import image3 from '../image/music3.webp';
...
const MusicClub = () => {
  const [hasMusicJoined, setHasJoined] = useState(false);
  ...
  const handleButtonClick = () => {
    if (hasMusicJoined) {
      ...
      axios.post('http://localhost:8080/api/musicleave-club')
        .then(response => {
      ...
      navigate('/musicjoin-club');
    }
  };

  // Fetch user status on component mount
  useEffect(() => {
    axios.get('http://localhost:8080/api/musiccheck-membership')
      .then(response => {
  ...
  return (
    <HeroSection>
      ...
      <MainContainer>
        <HeroContent>
          <HeroTitle>Welcome to the Music Club</HeroTitle>
        </HeroContent>

        <Section>
          <SectionContent>
            <Text>
            Discover your musical passion in our vibrant music club. Whether you're a singer, instrumentalist, or just love music, you'll find a place here. Participate in jam sessions, concerts, and workshops that cover everything from classical to contemporary music. Embrace the rhythm, learn new techniques, and share your musical journey with fellow enthusiasts.</Text>
            <JoinClubButton onClick={handleButtonClick}>
              {hasMusicJoined ? 'Leave Club' : 'Join Club'}
            </JoinClubButton>
          </SectionContent>
        </Section>
